...
def train_cir():
    ...
    input_size = 256
    iou_metric = 0
    ckp_dir = './ckp_'+str(input_size)
    os.makedirs(ckp_dir,exist_ok=True)
    criterion = DC_and_HDBinary_loss()
    shape_criterion = torch.nn.MSELoss()
    ...
    data_dataset = Datasets("./datasets/images/train","./datasets/labels/train",transform=x_transforms,target_transform=y_transforms,input_size = input_size)
    ...
    for epoch in range(num_epochs):
        ...
        for x, y in dataloaders:
            ...
            # print("reconstruction_crition--------")
            # print(pre_res_outputs.shape)
            # print(outputs_copy.detach().squeeze(dim=1).shape)
            loss_res1 = reconstruction_crition(pre_res_outputs,outputs_copy.detach().squeeze(dim=1))
            # print("reconstruction_critionres2--------")
            loss_res2 = reconstruction_crition(labels_res_outputs,blab.squeeze(dim=1))
        ...
        with torch.no_grad():
            data_dataset_test = Datasets("./datasets/images/val","./datasets/labels/val", transform=x_transforms,target_transform=y_transforms,input_size = input_size)
            dataloaders_test = DataLoader(data_dataset_test, batch_size=1)
            criterion = DC_and_HDBinary_loss()
            model = model.eval()
            iou,loss = test_model(model,criterion,dataloaders_test)
            print("val_iou:",iou,"val_loss:",loss)
            if iou>iou_metric:
                torch.save(model.state_dict(), ckp_dir + '/best_SGU_NetepochG.pth')
                torch.save(encoder.state_dict(), ckp_dir + '/best_SGU_NetepochDE.pth')
                torch.save(decoder.state_dict(), ckp_dir + '/best_SGU_NetepochDD.pth')
                iou_metric = iou
        ...
        torch.save(model.state_dict(),ckp_dir+'/SGU_Netepoch{}G.pth'.format(epoch+1))
        torch.save(encoder.state_dict(), ckp_dir+'/SGU_Netepoch{}DE.pth'.format(epoch + 1))
        torch.save(decoder.state_dict(), ckp_dir+'/SGU_Netepoch{}DD.pth'.format(epoch + 1))
